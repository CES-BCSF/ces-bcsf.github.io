...
nombres <- base::as.character(readxl::read_excel("../input/DF_paneles.xlsx", # NOMBRES DE LA SERIE
                                                 sheet = "mensuales",
                                                 range = "a1:i1", col_names = F)) # AL CARGAR UNA NUEVA SERIE, SE DEBE MODIFICAR EL RANGO PARA INCLURLA

# CARGAR Y PREPARAR LOS DATOS
base <- readxl::read_excel("../input/DF_paneles.xlsx", # 
                           sheet = "mensuales",
...
media <- readxl::read_excel("../input/DF_paneles.xlsx", # CALCULO DE MEDIA
...
desvio <- readxl::read_excel("../input/DF_paneles.xlsx", # CALCULO DE SD
                             sheet = "mensuales", col_names = T) |>
  dplyr::select(-Fecha) |>                
...
nombres_ia <- base::as.character(readxl::read_excel("../input/DF_paneles.xlsx", # NOMBRES DE LA SERIE
                                                 sheet = "i.a.",
                                                 range = "a1:i1", col_names = F)) # AL CARGAR UNA NUEVA SERIE, SE DEBE MODIFICAR EL RANGO PARA INCLURLA

# CARGAR Y PREPARAR LOS DATOS
base_ia <- readxl::read_excel("../input/DF_paneles.xlsx", # 
                           sheet = "i.a.",
                           range = "a362:i433", col_names = F) # 362 ES ENERO DE 2020 | AL CARGAR UNA NUEVA SERIE, SE DEBE MODIFICAR EL RANGO PARA INCLURLA
base::names(base_ia) <- nombres_ia

media_ia <- readxl::read_excel("../input/DF_paneles.xlsx", # CALCULO DE MEDIA
                            sheet = "i.a.", col_names = T) |>
  dplyr::select(-Fecha) |>                
  dplyr::summarise(dplyr::across(dplyr::everything(), ~ mean(.x, na.rm = TRUE)))|>
  tidyr::pivot_longer(dplyr::everything(), names_to = "Indicadores", values_to = "μ")

desvio_ia <- readxl::read_excel("../input/DF_paneles.xlsx", # CALCULO DE SD
                             sheet = "i.a.", col_names = T) |>
  dplyr::select(-Fecha) |>                
  dplyr::summarise(dplyr::across(dplyr::everything(), ~ sd(.x, na.rm = TRUE)))|>
  tidyr::pivot_longer(dplyr::everything(), names_to = "Indicadores", values_to = "σ")
