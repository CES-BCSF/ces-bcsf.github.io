...
archivos_a_borrar <- c(
  base::file.path(destino, "global_trimestral" , "correr_y_actualizar.lnk"),
  base::file.path(destino, "global_trimestral" , "templates", "Paneles_globales_trimestrales.R"),
  base::file.path(destino, "global_trimestral" , "templates", "salidas.r"),
  base::file.path(destino, "global_trimestral" , "templates", "injects.r"),
  base::file.path(destino, "global_trimestral" , "templates", "subir_paneles.R"),
  base::file.path(destino, "global_trimestral" , "input")
)
...
commit_message <- base::paste0("Auto-update: PANELES GLOBALES TRIMESTRALES actualizado al ", base::Sys.time())
...
base::tryCatch({ 
  ...
  gert::git_status() 
  gert::git_add("/web_ces/Paneles/global_trimestral")
  gert::git_commit(commit_message)
  #gert::git_branch_list()
  gert::git_push(remote = "origin") # LA NUBE
  base::message("Cambios en PANELES GLOBALES TRIMESTRALES subidos con Exito: ", commit_message)
}, error = function(e) {
  base::message("Error al subir: ", e$message)
})

base::setwd(path_viejo_gh)