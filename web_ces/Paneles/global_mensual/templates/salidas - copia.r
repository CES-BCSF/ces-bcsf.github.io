...
nombres <- base::as.character(readxl::read_excel(DF_paneles_path, # NOMBRES DE LA SERIE
                                                 sheet = "mensuales",
                                                 range = "a1:cl1", col_names = F)) # AL CARGAR UNA NUEVA SERIE, SE DEBE MODIFICAR EL RANGO PARA INCLURLA

# CARGAR Y PREPARAR LOS DATOS
base <- readxl::read_excel(DF_paneles_path, # 
                           sheet = "mensuales",
                           range = "a362:cl600", col_names = F) # 362 ES ENERO DE 2020 | AL CARGAR UNA NUEVA SERIE, SE DEBE MODIFICAR EL RANGO PARA INCLURLA
...
mi_css_2 <- base::readLines("../assets/css/styles-paneles-m.css")

dt_vm<- htmlwidgets::prependContent(dt_vm, htmltools::tags$style(htmltools::HTML(mi_css)))
dt_vm<- htmlwidgets::prependContent(dt_vm, htmltools::tags$style(htmltools::HTML(mi_css_2)))
dt_vm <- htmlwidgets::prependContent(dt_vm, htmltools::tags$script(htmltools::HTML(mi_script)))

htmlwidgets::saveWidget(
  dt_vm,
  file = "Tabla_global_VM.html",
  selfcontained = TRUE,
  title = NULL
)
...
# htmlwidgets::saveWidget(
#   dt_vm_mensual,
#   file = "../datatable_panel.html",
#   libdir = "datatable_panel_files_m",
#   selfcontained = FALSE,    # <---- IMPORTANTE PARA QUE NO ROMPA EL LAYOUT
#   title = NULL
# )

# frag <- readLines("../datatable_panel.html", warn = FALSE)
# 
...
nombres <- base::as.character(readxl::read_excel(DF_paneles_path, # NOMBRES DE LA SERIE
                                                 sheet = "i.a.",
                                                 range = "a1:ci1", col_names = F)) # AL CARGAR UNA NUEVA SERIE, SE DEBE MODIFICAR EL RANGO PARA INCLURLA

# CARGAR Y PREPARAR LOS DATOS
base <- readxl::read_excel(DF_paneles_path, # 
                           sheet = "i.a.",
                           range = "a362:ci600", col_names = F) # 362 ES ENERO DE 2020 | AL CARGAR UNA NUEVA SERIE, SE DEBE MODIFICAR EL RANGO PARA INCLURLA
base::names(base) <- nombres
...
dt_ia_m <- htmlwidgets::prependContent(dt_ia_m, htmltools::tags$style(htmltools::HTML(mi_css)))
dt_ia_m <- htmlwidgets::prependContent(dt_ia_m, htmltools::tags$style(htmltools::HTML(mi_css_2)))
dt_ia_m <- htmlwidgets::prependContent(dt_ia_m, htmltools::tags$script(htmltools::HTML(mi_script)))

htmlwidgets::saveWidget(
  dt_ia_m,
  file = "Tabla_global_mensual_IA.html",
  selfcontained = TRUE,
  title = NULL
)
