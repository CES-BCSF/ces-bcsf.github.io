...
destino <- "C:/mysyncfolders/bcsf.com.ar/BCSF - Grupo CES - Documentos/CicSFE_sp/_Reportes rmd/_Github_out/ces-bcsf.github.io/web_ces"   # <-- EDITAR AQUÍ
...
if (!dir.exists(dirname(destino))) {
  dir.create(dirname(destino), recursive = TRUE)
}
...
archivos_a_borrar <- c(
  file.path(destino, "Correr y actualizar paneles.lnk"),
  file.path(destino, "templates", "Paneles_icasfe.R"),
  file.path(destino, "templates", "salidas.r"),
  file.path(destino, "templates", "injects.r"),
  file.path(destino, "templates", "subir_paneles.R")
)
...
commit_message <- base::paste0("Auto-update: PANELES actualizado al ", base::Sys.time())
...
base::tryCatch({ 
  ...
  gert::git_status() 
  gert::git_add("/web_ces/Paneles")
  gert::git_commit(commit_message)
  #gert::git_branch_list()
  gert::git_push(remote = "origin") # LA NUBE
  base::message("Cambios en PANELES subidos con Exito: ", commit_message)
}, error = function(e) {
